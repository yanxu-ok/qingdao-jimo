...
function requestUrl(num) {
  request(_URL + "/app/article/common/json?pagesize=16&cateid=" + currectCateId + '&_orgid_=' + jiGouId + '&page=' + num + '&type=tv', 'Get', 'department', 'myHeader_contain');
}
...
// 當前位置
function position(templateId, data) {
  if (templateId == 'department') {
    t.eq(1).find('a').text('看电视' + '>');
